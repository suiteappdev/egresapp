 ...
 */
 ...
 angular
 .module('homer')
 .controller('descuentosCtrl', categoriasCtrl)
...
function categoriasCtrl($scope, $rootScope, api, menu, $modal, $stateParams, notify, sweetAlert) {
 ...
 $scope.new_record = function(){
     var modal = $modal.open({
         templateUrl: 'views/descuentos/new_record.html',
 ...
 $scope.edit_record = function(){
     ...
     $rootScope.modal = $modal.open({
         templateUrl: 'views/descuentos/edit_record.html',
         controller : 'descuentosCtrl'
     });
 }
 ...
 $scope.update = function(){
     if($scope.formRecordEdit.$valid){
         ...
         api.tipo_descuentos($rootScope.formEdit.id).put($rootScope.formEdit).then(function(response){
             $rootScope.loading  = false;
             $scope.load();
 ...
 $scope.save = function(){
     if($scope.formRecord.$valid){
         ...
         api.tipo_descuentos().post($scope.form.data).then(function(response){
             $scope.load();
             $rootScope.loading = false;
             $scope.homerTemplate = 'views/notification/notify.html';
 ...
 $scope.remove  = function(id){
     api.tipo_descuentos(id).delete().then(function(response){
         $scope.load();
         $rootScope.loading = false;
         sweetAlert.swal("Borrado!", "El descuento ha sido borrado correctamente", "success");
     }).catch(function(e){
         console.log(e.message);
     }); 
 }
